...
enum {
	INIT_HOMING, PID_TEST, PHOTI_LIMIT, IDLE
} State = INIT_HOMING;
/* USER CODE END PV */
...
//		Test_Range();
		read_pos();
		static uint32_t timestamp_traject = 0;
		switch (State) {
		case INIT_HOMING:
			Init_Homing();
			break;
		case PID_TEST:
			if (HAL_GetTick() >= timestamp_traject) {
				timestamp_traject = HAL_GetTick() + 1;
				switch (State_PID) {
				case 0:
...
static void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pins : B1_Pin Photoelectric_sensor_3_Pin Photoelectric_sensor_2_Pin */
	...
	/*Configure GPIO pins : Joy1_Pin Joy2_Pin Joy3_Pin Joy4_Pin
	 Joy5_Pin Joy6_Pin Joy7_Pin */
	GPIO_InitStruct.Pin = Joy1_Pin | Joy2_Pin | Joy3_Pin | Joy4_Pin | Joy5_Pin
			| Joy6_Pin | Joy7_Pin;
	...
	/*Configure GPIO pins : Joy9_Pin EmergencySwitch_Pin */
	GPIO_InitStruct.Pin = Joy9_Pin | EmergencySwitch_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	...
	/*Configure GPIO pin : Photoelectric_sensor_1_Pin */
	GPIO_InitStruct.Pin = Photoelectric_sensor_1_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(Photoelectric_sensor_1_GPIO_Port, &GPIO_InitStruct);
...
//	static uint32_t timestamp_pid = 0;
//	if (HAL_GetTick() >= timestamp_pid) {
//
//		timestamp_pid = HAL_GetTick() + 1;
	static int overshoot_check = 0;
...
//	}
	...
	if (pos_f >= PosY - 0.2 && pos_f <= PosY + 0.2 && overshoot_check == 1) {
...
//		if (setposition < 0) {
//			setposition = 0;
//		} else if (setposition > Max_Workspace) {
//			setposition = Max_Workspace;
//		}
//
...
//
//		Error = setposition - PosY;
//		if (!((velocityfeedback >= max_velocity)
//				&& ((Error >= 0 && Intregral >= 0)
//						|| (Error < 0 && Intregral < 0)))) {
//			Intregral = Intregral + Error;
//		}
//
//		velocityfeedback = (Kp_pos * Error)
//				+ (Kd_pos * ((Error - Last_Error) / deltaT))
//				+ (Intregral * Ki_pos);
//
//		Last_Error = Error;
...
//
//		if (velocityfeedback > max_velocity) {
//			velocityfeedback = max_velocity;
//		} else if (velocityfeedback < -max_velocity) {
//			velocityfeedback = -max_velocity;
//		}
//	}
//}
...
//		if (!((setvelocity >= Max_Velocity)
//				&& ((Error >= 0 && Intregral >= 0)
//						|| (Error < 0 && Intregral < 0)))) {
//			Intregral = Intregral + Error;
//		}
//
//		Dutyfeedback = (Kp_velo * Error)
//				+ (Kd_velo * ((Error - Last_Error) / deltaT))
//				+ (Intregral * Ki_velo);
//
//		if (Dutyfeedback < 0) {
//			dir = 1;
...
//
//		if (Dutyfeedback > Max_Counter_PWM) {
//			Dutyfeedback = Max_Counter_PWM;
//		} else if (Dutyfeedback < - Max_Counter_PWM) {
//			Dutyfeedback = -Max_Counter_PWM;
//		}
//
...
void motor(uint32_t speed, int DIR) {
	...
	if (DIR == -1) {
		...
		//			  __HAL_	IM_SET_COMPARE(&htim1,TIM_CHANNEL_1,65535*0.7);
	} else if (DIR == 1) {
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	...
	} else if (GPIO_Pin == Photoelectric_sensor_3_Pin) {
		if (State == PID_TEST) {
			__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 0);
			state_IT = 0;
			State = PHOTI_LIMIT;
